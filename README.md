...
`content.js` queries claude.ai's own API (`/api/organizations/{id}/chat_conversations` and `/api/organizations/{id}/projects`) with the user's signed-in session, then hides matching `<li>` elements in the sidebar with `style.display = 'none'`. A MutationObserver keeps hidden chats hidden as the sidebar updates.
